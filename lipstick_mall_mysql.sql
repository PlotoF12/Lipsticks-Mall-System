...
-- -----------------------------------------------------------
-- 5. 用户偏好表（长期记忆）
--    从 AI 对话中异步提取的结构化偏好，用于个性化推荐
--    与 user_profile 的区别：
--      user_profile 是用户手动填写的画像（性别/肤色/肤质）
--      user_preference 是系统从对话中自动学习的偏好（色系/质地/品牌等）
-- -----------------------------------------------------------
DROP TABLE IF EXISTS user_preference;
CREATE TABLE user_preference (
    id               BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
    username         VARCHAR(64)  NOT NULL                COMMENT '用户名，关联 user_account.username',
    preferred_colors VARCHAR(128)                         COMMENT '偏好的色系，逗号分隔: nude,bean_paste',
    preferred_finish VARCHAR(32)                          COMMENT '偏好的质地: matte / gloss / satin',
    preferred_brands VARCHAR(256)                         COMMENT '偏好的品牌，逗号分隔: Dior,MAC',
    price_min        INT                                  COMMENT '价格区间下限（元）',
    price_max        INT                                  COMMENT '价格区间上限（元）',
    disliked_colors  VARCHAR(128)                         COMMENT '不喜欢的色系，逗号分隔',
    scenes           VARCHAR(256)                         COMMENT '常用场景，逗号分隔: 日常,通勤',
    lip_condition    VARCHAR(32)                          COMMENT '唇部状况: dry / normal',
    updated_at       DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_username (username),
    CONSTRAINT fk_pref_username FOREIGN KEY (username) REFERENCES user_account (username) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='用户偏好表（长期记忆）';
