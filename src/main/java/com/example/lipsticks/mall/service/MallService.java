...
public class MallService {
    ...
    /**
     * @param redisTemplate 可选 —— 测试环境（H2）中 Redis 不可用时为 null，
     *                      此时跳过缓存，直接查数据库
     */
    public MallService(LipstickProductMapper lipstickProductMapper,
                       ObjectMapper objectMapper,
                       @Autowired(required = false) StringRedisTemplate redisTemplate) {
        this.lipstickProductMapper = lipstickProductMapper;
    ...
    public Page<LipstickProduct> pageProducts(int pageNum, int pageSize,
        ...
        if (!hasFilter && pageNum == 1 && pageSize == 6 && redisTemplate != null) {
